...
export default function ProductItemsInSearch() {
  //
  //
  // declare context :
  ...
  const handleShowModal = (ID) => {
    ...
    ContextData.setSelectedProduct(
      ContextData.searchedProducts.find((product) => product.id === ID)
    );
  };
  //
  //
  ...
  const addProductToCart = (ID) => {
    ...
    let addedProduct = ContextData.searchedProducts.find(
  ...
  return (
    <div className="product-items">
      <Row className="justify-content-center" sm={12} md={6} lg={3}>
        {ContextData.searchedProducts.map((productItem) => (
          <Card
            style={{ width: "18rem" }}
            key={productItem.id}
